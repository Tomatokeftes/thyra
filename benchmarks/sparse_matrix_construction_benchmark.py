...
class SparseMatrixBenchmark:
    ...
    def benchmark_lil_matrix(self) -> Dict[str, Any]:
        ...
        for pixel_idx, mz_idx, intensities in self.generate_synthetic_data():
            lil[pixel_idx, mz_idx] = intensities

            # Log progress every 100k pixels
            if (pixel_idx + 1) % 100_000 == 0:
                now = time.time()
                ...
                print(
                    f"  {pixel_idx+1:8,} pixels | "
                    f"Interval: {interval_rate:6.0f} spec/s | "
                    f"Overall: {overall_rate:6.0f} spec/s | "
    ...
    def generate_report(self, results: List[Dict[str, Any]]) -> None:
        ...
        # Tabular results
        print(f"{'Method':<25} {'Time (s)':>10} {'Memory (GB)':>12} {'Rate (spec/s)':>15}")
        print("-" * 70)
